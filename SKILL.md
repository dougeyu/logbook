...
- **绝不跳过本 skill**:脚本类工具(python 等)是可选的增强,不是必需前提。脚本因权限或沙箱无法运行时,必须降级为「读取本地文件 + 用户口述」,照常走完整流程。任何情况下不得因脚本不可用而跳过本 skill 或放弃证据驱动原则——拿不到脚本证据时靠读文件和追问补齐,而不是绕过 skill 去编造。
...
脚本无法运行时(如沙箱禁止 python):不得因此跳过本 skill。改用手动方式——直接读取 skill 目录下 config.json(如存在)判断四项;缺失则逐一向用户确认四项,确认后在对话中记录或用文件工具手动创建/更新 config.json。四项仍是必答,只是把「跑脚本」换成「读文件 + 问答」。
...
主路是「读文件 + 用户口述」,脚本只是可选的自动增强。脚本跑不了时,读文件和追问仍然能拿到完整证据,照常继续。

按此优先级:

1. 当前用户消息与附件。
2. 对话中提供的历史报告或笔记。
3. 明确授权的只读工具或数据源(脚本能跑时)。
4. 针对重大缺口的聚焦追问。
...
- 脚本/工具不可用或权限不足(如沙箱禁止 python/node/git):**绝不跳过本 skill**。明确说明哪个脚本跑不了,改为直接读取本地文件 + 追问用户,用文件内容和口述继续收集证据,照常走完规范化、撰写、保真检查流程。
